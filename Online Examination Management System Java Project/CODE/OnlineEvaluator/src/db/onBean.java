...
returnvalue.put(rno,rows);
	...
	public Hashtable getPaymentDtl(String bid)
	{
		...
		try{
			Statement st1=con.createStatement();
			String qry="select a.payid,a.payamount,a.paydate,a.paymode,a.cust_id from paymentmaster a,customer b where a.cust_id=b.cust_id and b.batchid='"+bid+"'";
...
public Hashtable getCustomerDetails(String id)
	{
		Hashtable returnvalue=new Hashtable();
		String query="select CUST_ID,CUST_NAME,CUST_ADDRESS,CUST_CITY,CUST_STATE,CUST_COUNTRY,CUST_FAX1,CUST_DOB,CUST_MAIL,SEX,CUST_RESNO,CUST_OFFNO,CUST_MOBILE,CUST_ADD2,BATCHID from customer where cust_id='"+id+"'";
		System.out.println(query);
		try{
		Statement st1=con.createStatement();
		ResultSet rs=st1.executeQuery(query);
...
public Hashtable getBatchDetails(int id)
	{
		...
		String query="select BATCHID,NAME,BDESC,STARTDATE,ENDDATE,COURSEID,TUTORID,STRENGTH from batch where BATCHID="+id+"";
		System.out.println(query);
		try{
		Statement st1=con.createStatement();
		ResultSet rs=st1.executeQuery(query);
...
public Hashtable getBatches()
	{
		...
		ResultSet rs=st.executeQuery("select BATCHID,NAME,BDESC,STARTDATE,ENDDATE,COURSEID,TUTORID,STRENGTH from batch");
		while(rs.next())
			{
...
//		  month=month;
		...
		else
		{
			...
			ResultSet rs = st.executeQuery("SELECT BATCHID FROM CUSTOMER WHERE CUST_ID='"+id+"'");
			String n="SELECT BATCHID FROM CUSTOMER WHERE CUST_ID='"+id+"'";
			System.out.println("--n---"+n);
			if (rs.next())
			{
				bid = rs.getInt(1)+"";
				System.out.println("--bid--"+bid);
				String s="SELECT BATCHID FROM ONLINE_TESTLOGIN WHERE EXMDATE LIKE '"+curdate+"' AND BATCHID="+bid+" AND CUST_ID='"+id+"' AND STATUS='NOTTAKEN'";
				System.out.println("--query--"+s);
				ResultSet rs2 = st2.executeQuery("SELECT BATCHID FROM ONLINE_TESTLOGIN WHERE EXMDATE LIKE '"+curdate+"' AND BATCHID="+bid+" AND CUST_ID='"+id+"' AND STATUS='NOTTAKEN'");
				System.out.println("In Method...");
				if(!rs2.next())
				{
					bid = null;
